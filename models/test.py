...
def error_tests(ctmc_gen, dtmc_gen):
    for param in [0.01, 0.1]:
        node = 1
        node2 = 2
        for n in [3]:
            print "Error tests (model checker)"
            ctmc = ctmc_gen(n,param)
            dtmc = dtmc_gen(n,param)
            test_equalish(prism.run(ctmc, node_dist(node, n))[1:], tolerance=0.02)
            test_equalish(prism.run(ctmc, remove_roots(node_inter(node, node2, n),n)), tolerance=0.02)
            test_equalish(prism.run(dtmc, node_dist(node, n))[1:], tolerance=0.02)
            test_equalish(prism.run(dtmc, remove_roots(node_inter(node, node2, n),n)), tolerance=0.02)
            test_equalish(prism.run(dtmc, remove_roots(node_past(node, n),n)), tolerance=0.02)
        for n in [5,10,20]:
            print "Error tests (sim)"
            ctmc = ctmc_gen(n,param)
            dtmc = dtmc_gen(n,param)
            test_equalish(prism.sim(ctmc+node_dist_reward(node, n), node_dist_r(node, n)[1:]), tolerance=0.02)
            test_equalish(prism.sim(ctmc+node_inter_reward(node, node2, n), remove_roots(node_inter_r(node, node2, n))), tolerance=0.02)
            test_equalish(prism.sim(dtmc+node_dist_reward(node, n), node_dist_r(node, n)[1:]), tolerance=0.02)
            test_equalish(prism.sim(dtmc+node_inter_reward(node, node2, n), remove_roots(node_inter_r(node, node2, n))), tolerance=0.02)
            test_equalish(prism.sim(dtmc+node_past_reward(node, n), remove_roots(node_past_r(node, n))), tolerance=0.02)
...
if __name__ == '__main__':
    all_tests()
    #error_tests(ctmc_full_error, dtmc_full_error)
    #base_tests_sim(ctmc_full_churn)
